  ...
  return (
    <div className="min-h-screen pt-24 pb-16 px-6">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold mb-2 text-foreground">My Results</h1>
        <p className="text-muted mb-12">Your saved career and major quiz results</p>

        <div className="space-y-12">
          <section>
            <h2 className="text-xl font-semibold mb-6 text-accent-primary">Career Quiz Results</h2>
            {careerResults && careerResults.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(result.top_careers || []).slice(0, 5).map((c: { name: string }) => (
                        <span
                          key={c.name}
                          className="px-3 py-1 rounded-lg bg-accent-primary/10 text-accent-primary text-sm font-medium"
                        >
                          {c.name}
                        </span>
                      ))}
              ...
              <div className="p-8 rounded-2xl bg-card border border-border shadow-soft text-center text-muted">
          ...
          <section>
            ...
            {majorResults && majorResults.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(result.top_majors || []).slice(0, 5).map((m: { name: string }) => (
                        <span
                          key={m.name}
                          className="px-3 py-1 rounded-lg bg-accent-purple/10 text-accent-purple text-sm font-medium"
                        >
                          {m.name}
                        </span>
                      ))}
              ...
              <div className="p-8 rounded-2xl bg-card border border-border shadow-soft text-center text-muted">
                ...
                <Link
                  href="/major-quiz"
                  className="inline-block px-6 py-3 rounded-xl bg-accent-purple text-white font-medium hover:bg-accent-secondary transition"
                >
                  Take Major Quiz
                </Link>
              </div>
